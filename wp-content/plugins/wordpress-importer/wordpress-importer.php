...
//mod jrc 300312 - more reliable matching - esp. for wpmu 2.8 paths - eg: 'files/2009/12/myfile.jpg' (note: no leading '/') Vs 'http://dmionline.net/files/2009/12/myfile.jpg' - as output by advanced-export-for-wp-wpmu plugin (http://wordpress.org/extend/plugins/advanced-export-for-wp-wpmu/) working on wpmu V2.8/V2.9 and earlier
//orig:
...
//mod jrc 050412 - also prepare to substitute any stray relative paths to the attachment, as well as absolute paths found in content by backfill_attachment_urls
//note: if importing multiple batches be sure to a) load ALL posts before attatchments b) import all attachments as single batch, (if at all possible).
//orig:
/*
                        $parts = pathinfo( $url );
...
*/
//new:
                        $url_from_parts = parse_url($url);
                        $path_from_parts = pathinfo($url_from_parts['path']);
                        if (!defined('PATHINFO_FILENAME')) $path_from_parts['filename'] = basename( $path_from_parts['basename'], '.' . $path_from_parts['extension'] ); //php < 5.2 fix for pathinfo
 
                        $url_from_str = (isset($url_from_parts['scheme']))? $url_from_parts['scheme'] . '://' . $url_from_parts['host'] . $path_from_parts['dirname'] . '/' . $path_from_parts['filename'] : '';
                        $path_from_str = ltrim($path_from_parts['dirname'] . '/' . $path_from_parts['filename'],'/');
 
                        $url_to_parts = parse_url($upload['url']);
                        $path_to_parts = pathinfo($url_to_parts['path']);
                        if (!defined('PATHINFO_FILENAME')) $path_to_parts['filename'] = basename( $path_to_parts['basename'], '.' . $path_to_parts['extension'] ); //php < 5.2 fix for pathinfo
 
                        $url_to_str = (isset($url_to_parts['scheme']))? $url_to_parts['scheme'] . '://' . $url_to_parts['host'] . $path_to_parts['dirname'] . '/' . $path_to_parts['filename'] : '';
                        $path_to_str = ltrim($path_to_parts['dirname'] . '/' . $path_to_parts['filename'],'/');
 
                        if (!empty($url_from_str)) $this->url_remap[$url_from_str] = $url_to_str;
                        if (strcmp($path_from_str,$path_to_str) !== 0) $this->url_remap[$path_from_str] = $path_to_str;
//end mod jrc 050412
                }
